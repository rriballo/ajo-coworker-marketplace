...
6. If only a Journey ID is known, call `ajo_journey_resolve_campaigns` and require an exact action-node or campaign selection when multiple actions are returned. Composite Journey-inline campaign IDs are valid for authored message reads and do not alone indicate an Orchestrated campaign. Ask Coworker for the selected Action `campaignVersionId` when it is not present in the Journey result. Resolve the target Decision Policy scope with `ajo_campaign_resolve_scope`; root IDs remain a compatibility fallback. If package or message selection is ambiguous, ask for the exact IDs and retry the resolver.
...
6. After content is assembled, resolve the Journey first with `ajo_journey_resolve_campaigns` when the delivery identifiers are not already known. Never choose silently when `selectionRequired` is true. Use the selected candidate's exact `campaignId` and `messageId` with `ajo_campaign_get_message_content` and report every authored subject, HTML, text, and variant result. Then call `ajo_campaign_preview_content` only when the resolver returns `simulationPreview.supported: true` with a non-null `previewCampaignId` and a test profile is available. Authored content is not profile-personalized rendering, and neither operation simulates the Journey path, branching, waits, events, consent, eligibility, proof, or delivery. Full Journey Simulation, proofing, and activation remain external AJO steps.
