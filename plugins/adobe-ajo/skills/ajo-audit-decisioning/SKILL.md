...
5. Inspect every in-scope strategy and verify collection, rule, formula, ranking, and candidate-item dependencies. For audience-backed rules, report exact `inSegment` system IDs and AND/OR semantics without claiming member qualification; missing Adobe `segmentModel` evidence affects visual editing, not proof that persisted PQL was rejected.
