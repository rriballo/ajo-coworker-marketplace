...
4. Validate item readiness and distinguish structural readiness, approval state, date-window eligibility, stored rule attachment, and unsimulated delivery eligibility. Rule existence is not attachment evidence; verify the exact item constraint path.
5. Inspect every in-scope strategy and verify collection, rule, formula, ranking, and candidate-item dependencies. Call `ajo_decisioning_verify_rule` for in-scope eligibility rules. For schema-backed structured rules, verify the exact Profile schema and returned `pqlPath` evidence; never use the Offer Item schema. For audience-backed rules, report exact `inSegment` IDs and AND/OR semantics without claiming member qualification. Keep persistence, visual-model, attachment, and simulation evidence separate.
